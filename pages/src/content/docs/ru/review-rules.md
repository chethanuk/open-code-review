...
- `include` — необязательно. Glob-шаблоны, которые *обходят* встроенные
  ...
  отревьюены. Файл, отброшенный на `binary` или `undecodable_encoding`,
  шаблон `include` не спасёт: эти проверки идут раньше.
...
2. **`undecodable_encoding`** — Байты читаются как текст хотя бы в одной
   поддерживаемой кодировке? Исключается, если нет.
3. **`secret_exclude`** — Старый или новый путь подпадает под встроенную защиту секретных путей? Безусловные glob-шаблоны перечислены в [`default_secret_patterns.json`](https://github.com/alibaba/open-code-review/blob/main/internal/config/allowlist/default_secret_patterns.json). Если да, путь исключается.
...
4. **`user_exclude`** — Путь совпадает с каким-либо пользовательским шаблоном
   `exclude`? Исключается.
5. **`user_include`** — Если пользователь задал `include`, путь совпадает? Если
   да, **сразу остаётся** (обходит проверки `unsupported_ext` и `default_path`
   ниже).
6. **`unsupported_ext`** — Расширение файла есть в
...
7. **`default_path`** — Путь совпадает со встроенным шаблоном исключения
   тестовых файлов (`**/*_test.go`, `**/*.test.{js,jsx,ts,tsx}`,
   `**/*_spec.rb`, …)? Исключается.

Файлы, прошедшие все семь проверок, отправляются в LLM декодированным UTF-8
текстом, а не сырыми байтами, если только сам diff не превышает 80% от
`max_tokens`: `selectFiles` применяет этот предел после проверок и исключает
файл как `too_large`. Он же помечает файл, чей новый путь — `/dev/null`,
причиной `deleted`; нового содержимого для ревью нет. Используйте
`ocr review --preview`, чтобы вывести результат этого фильтра, не тратя ни
одного токена.
