...
| `--on-grouping-failure <mode>` | — | `fallback` | Что делает неудавшийся вызов группировки LLM: `fallback` — ревью каждого файла по отдельности с предупреждением `grouping_failed`; `abort` — остановка ревью с кодом выхода 1. |
...
| `1` | Критическая ошибка — неверные флаги, не удалось определить эндпоинт LLM, ошибкой завершились все субагенты по файлам, вызов группировки LLM завершился ошибкой при `--on-grouping-failure=abort` и т. п. Текст ошибки выводится в stderr. |
