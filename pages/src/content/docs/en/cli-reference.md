...
| `--max-tokens-budget <n>` | — | `0` (unlimited) | Cap total input + output token usage for the review. Overrides the saved `max_tokens_budget` setting for this run. Checked before every LLM round: a subtask already over budget gets one final round to submit findings and is reported as `failed(budget)`, no further subtasks are dispatched, and partial results are still published. |
...
- **`unset`** — clear a saved key. `provider`, `max_tokens`,
  `max_tokens_budget`, `effort`, `custom_providers.<name>`, and
  `mcp_servers.<name>` are supported.
