...
| `--on-grouping-failure <mode>` | — | `fallback` | LLM グルーピング呼び出しが失敗したときの動作。`fallback` は各ファイルを個別にレビューし、`grouping_failed` 警告を記録します。`abort` は終了コード 1 でレビューを停止します。 |
...
| `1` | 致命的エラー。引数の誤り、LLM エンドポイントを解決できない、すべてのファイルごとのサブエージェントが失敗した、`--on-grouping-failure=abort` で LLM グルーピングが失敗した、などです。エラーテキストは stderr に出力されます。 |
