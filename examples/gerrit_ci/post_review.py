...
def fold_comments(review_input):
    ...
    for path, entries in (review_input.get("comments") or {}).items():
        for e in entries:
            ...
            parts.append("\n---\n\n%s\n\n%s" % (loc, e["message"]))
    folded["message"] = truncate_message("\n".join(parts))
    return folded
...
def main(argv=None):
    ...
    revision = args.revision or env("GERRIT_PATCHSET_REVISION", "") or "current"
    ...
    try:
        ...
        log("error: cannot read review result %s: %s" % (args.input, e))
        return 1
