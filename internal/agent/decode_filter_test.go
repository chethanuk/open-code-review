...
// filterDiffsForTest is the selection-and-report pass Run performs, in the one
// call these tests assert on: select, report, keep. Selection and reporting are
// separate functions in the run itself, so this is where the two are put back
// together for a test that is about what the user sees.
func (a *Agent) filterDiffsForTest(diffs []model.Diff) []model.Diff {
	decisions := a.selectFiles(diffs)
	a.logExclusions(decisions)
	kept, _ := summarizeSelection(decisions)
	return kept
}
...
func TestFilterDiffsEncodingReporting(t *testing.T) {
	...
	t.Run("D8_marked_but_still_reviewed", func(t *testing.T) {
		...
		warn = captureStderr(t, func() { diffs = a.filterDiffsForTest(diffs) })
		restore()
	...
	t.Run("D10_excluded_message_names_the_encoding", func(t *testing.T) {
		...
		var out bytes.Buffer
		restore := stdout.Swap(&out)
		kept := a.filterDiffsForTest(diffs)
		restore()
	...
	t.Run("existing_skip_messages_unchanged", func(t *testing.T) {
		...
		var out bytes.Buffer
		restore := stdout.Swap(&out)
		a.filterDiffsForTest([]model.Diff{
