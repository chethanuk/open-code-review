...
// filterScanItemsForTest is the selection-and-report pass Run performs, in the
// one call these tests assert on: select, report, keep. Selection and reporting
// are separate functions in the run itself, so this is where the two are put
// back together for a test that is about what the user sees.
func (a *Agent) filterScanItemsForTest(items []model.ScanItem) []model.ScanItem {
...
func TestFilterScanItemsEncodingReporting(t *testing.T) {
	...
	t.Run("D8_marked_but_still_reviewed", func(t *testing.T) {
		...
		warn := captureStderr(t, func() {
			kept = a.filterScanItemsForTest([]model.ScanItem{
