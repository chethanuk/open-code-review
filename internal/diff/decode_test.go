...
func TestDecodeFrameShapes(t *testing.T) {
	...
	// CRLF diff text reaches the decode already normalized: splitDiffLines
	// strips each line's trailing carriage return so it cannot ride into the
	// "diff --git" path capture. What this pins is that the decode works on
	// that normalized text — every line kept, the payload decoded, and no CR
	// put back.
	//
	// The fixture is the CRLF form of gbkFileNew rather than a three-line file
	// of its own: a file that short detects from too little evidence, so it
	// would be testing the detector instead of CRLF handling.
	t.Run("F11_crlf_input_decodes", func(t *testing.T) {
		const name = "auth/token.go"
		const gbkLine0 = "// \u7528\u6237\u8BA4\u8BC1\u6A21\u5757\uFF0C\u8D1F\u8D23\u6821\u9A8C\u4EE4\u724C\u5E76\u5237\u65B0\u4F1A\u8BDD\u72B6\u6001\u3002"
		const gbkLine1 = "// \u5982\u679C\u4EE4\u724C\u5DF2\u7ECF\u8FC7\u671F\uFF0C\u8C03\u7528\u65B9\u9700\u8981\u91CD\u65B0\u767B\u5F55\u83B7\u53D6\u65B0\u7684\u51ED\u8BC1\u3002"

		dir := t.TempDir()
		crlf := strings.ReplaceAll(gbkFileNew, "\n", "\r\n")
		writeRepoFile(t, dir, name, encodeFixture(t, simplifiedchinese.GBK, crlf))
		...
		d := diffs[0]
		if n := strings.Count(d.Diff, "\r"); n != 0 {
			t.Errorf("the decode put %d CR(s) back after the parser stripped them:\n%q", n, d.Diff)
		}
		...
		if !strings.Contains(d.Diff, "+"+gbkLine1) {
			t.Errorf("a CRLF-terminated hunk line was not decoded:\n%s", d.Diff)
		}
	})
}
